...
def detect_unit_suggestion(bodies: List[Body]) -> str:
    """Return the unit that the UI should display after loading a STEP file.

    ``load_step`` already converts every body mesh to millimetres via
    ``_choose_mm_scale``, so the UI baseline must be ``mm`` regardless of the
    original STEP unit.  Any further user change in the UI is then applied by
    ``apply_unit_scale`` relative to this millimetre baseline.
    """
    return "mm"
